#!/usr/bin/env python 
import ros
import sys
import rospy 
import cv2
from sensor_msgs.msg import Image
from cv_bridge import CvBridge, CvBridgeError
from matplotlib import pyplot as plt

import pytesseract
from PIL import Image 
from std_msgs.msg import String


#huff transform -detect arrow --(direction is poiniting ) center of mass -- semgementation ->grid what is each grid

class image_converter:

  def __init__(self):
  
    self.bridge = CvBridge()
    self.stringpub = rospy.Publisher('new_message',String, queue_size =10)
    self.string_data = ""
    #self.image_sub = rospy.Subscriber("/camera/image",Image,self.callback)
    #self.image_sub = rospy.Subscriber("/raspicam_node/image/compressed",Image,self.callback)
  
  ''' for line 25, subscribe call call back for captured image from pi
  def callback(self,data):
    try:
      print("runs")
      cv_image = self.bridge.imgmsg_to_cv2(data, "bgr8")
      #print("runs")
      #cv2.imwrite('camera_image.jpeg',cv_image)      
    
 
      # Set threshold and maxValue
      #thresh = 0
      #maxValue = 255
      # Basic threshold example
      #th, dst = cv2.threshold(src, thresh, maxValue, cv2.THRESH_BINARY)
  
    except CvBridgeError as e:
      print(e)
    #print('sub')
    #imshow("cam",cv_image)
  
     
    cv_image = cv2.cvtColor(cv_image, cv2.COLOR_BGR2GRAY) # Convert to grayscale
    cv_image = cv2.blur(cv_image,(50,50))	 # Blur image
    cv_image = cv2.Canny(cv_image,50,150)

    cv2.imshow("Image window", cv_image)
   
    cv2.waitKey(3)
  '''
	
def main(args):
  ic = image_converter()
  rospy.init_node('test_cv', anonymous=True)
  try:

    
    #reading from raw image and fixing multiples of 32 pixels and sacing it
    #as a new file  
    ###input image
    src = cv2.imread("text.jpg", cv2.IMREAD_COLOR)
    cv2.imshow("new",src)
    cv2.waitKey(2)
    #resizing by multiples of 32x32
    height, width = src.shape[:2]
    x = height
    y = width
    if height % 32 != 0:
      x = height -height%32
    if width % 32 !=0:
      y = width - height %32

    resized = src[0:x, 0:y]
    cv2.waitKey(2)
    '''##writing resized file 
    cv2.imwrite("stop_sign2.jpg", resized)
	
    cv2.waitKey(2)
    #src = cv2.imread("image_cam_image.png")
    #src = cv2.imread("large.jpg")
    #src = cv2.imread('stop_sign.png')
    
    #text = pytesseract.image_to_string(Image.open('new_file.png'))
    #cv2.imwrite("new_file_gray.png",sr c)
    #cv2.waitKey()
    
    '''
    #use resized image
    src = resized 
    cv2.waitKey(2)
    ##READ PROCESSED(CROPPED) AND NEWLY SAVED FILE
    #src = cv2.imread(resized, cv2.IMREAD_COLOR)
    #src = cv2.cvtColor(src,cv2.COLOR_BGR2GRAY)
    #cv2.imshow("new",src)
    #cv2.waitKey(2)
    
    
    ##threshold setting
    threshold_val = 200 # values below thresholdvalue in script heading
    max_val = 255 #values above thresholdVal will be set to white
    th, modimg = cv2.threshold(src,threshold_val, max_val, cv2.THRESH_BINARY)
    #cv2.imshow("threshold_image",modimg)
    #cv2.waitKey(2)
    cv2.waitKey(2)
    ##blur
    #modimg = cv2.blur(modimg,(5,5))
    #modimg = cv2.GaussianBlur(modimg,(5,5),0)
    #cv2.imshow("threshold_image",modimg)
    #cv2.waitKey(2)   
    
    text = str( pytesseract.image_to_string(src, lang="eng") )
    while not rospy.is_shutdown():
      ic.string_data = text
      print (text)
      ic.stringpub.publish(ic.string_data) #publish string message with topic
      rospy.sleep(1)
    #rospy.spin() for any subscribe
  except KeyboardInterrupt:
    print("Shutting down")
  cv2.destroyAllWindows()   
    

if __name__ == '__main__':
    main(sys.argv)
