...
# Fixed perspective transform and now published with no issue 10 April 11:35pm -Jay Lee
...
# insert jays code
        ...
        if lines is not None:
            ...
            LAST_LINES_AVG = lines_avg
            img_lines = line_display(image_copy, lines_avg)# back to lines from lines_avg
            #print("lines_avg" + str(lines_avg))
#            for line in lines_avg:
#                print("Line Avg: " + str(line))
            img_combined = img_canny_detection
            img_combined = cv2.addWeighted(image_copy, 0.7, img_lines, 1, 1)
            rows, cols, ch = img_combined.shape
        else:
 #           for line in LAST_LINES_AVG:
 #               print("Last Line: " + str(LAST_LINES_AVG))
            img_lines = line_display(image_copy, LAST_LINES_AVG)# back to lines from lines_avg # from LAST_LINES_AVG to lines_avg
            img_combined = cv2.addWeighted(image_copy, 0.7, img_lines, 1, 1)
            rows, cols, ch = img_combined.shape
